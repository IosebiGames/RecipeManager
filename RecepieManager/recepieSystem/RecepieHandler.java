...
public class RecepieHandler {
	...
	public void writeRecepie(String fileName, String recepie) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))){
			  writer.write(recepie);
			  writer.close();
		}catch(IOException e) {
			IO.println("Can't write Recepie: " + e.getMessage());
		}
	}
	public void writeAllergen(String AllergenFileName, String possibleAllergens) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(AllergenFileName))){
			  writer.write(possibleAllergens);
			  writer.close();
		}catch(IOException e) {
			IO.println("Can't write Recepie: " + e.getMessage());
		}
